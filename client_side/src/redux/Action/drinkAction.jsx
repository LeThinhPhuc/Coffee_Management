...
import { addDrinkSuccess, addDrinkFailure, deleteDrinkSuccess, deleteDrinkFailure, updateDrinkSuccess, updateDrinkFailure, fetchDrinksSuccess, fetchDrinksFailure } from '../Reducer/drinkSlice';
...
export const addDrink = (DrinkData) => {
  return async (dispatch) => {
    try {
      ...
      // Dispatch action addDrinkSuccess với dữ liệu người dùng mới được thêm
...
export const updateDrink = (DrinkId, updatedDrinkData) => {
  return async (dispatch) => {
    try {
      ...
      const response = await fetch(`https://api.example.com/Drinks/${DrinkId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      dispatch(updateDrinkSuccess({ id: DrinkId, updatedDrinkData: data }));
    } catch (error) {
