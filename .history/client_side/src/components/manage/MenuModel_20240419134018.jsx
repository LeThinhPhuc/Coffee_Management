...
const MenuModel = (props) => {
    ...
    useEffect(() => {
        // Cập nhật giá trị mặc định khi component được render lần đầu tiên
        if (types.length > 0) {
            setCategory(types[0].id); // Chọn id của option đầu tiên
        }
    }, [types]); // Chạy lại khi types thay đổi
    // console.log(types);
    ...
    return (
        <div>
            ...
            <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-40 outline-none focus:outline-none transition-all custom-animate-pulse">
                <div className="relative w-auto overflow-hidden my-6 mx-auto max-w-3xl h-screen overflow-hidden">
                    {/*//*  */}
                    <div
                        className="z-50 fixed right-10 top-10 flex items-center p-4 mb-4 text-sm text-yellow-800 border border-yellow-300 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300 dark:border-yellow-800"
                        role="alert"
                    >
                        <svg
                    ...
                    {/*//! content*/}
                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none h-screen overflow-hidden">
                        {/*header*/}
