...
export const AppProvider = ({ children }) => {
  ...
  const addSelectedDrink = (newSelected) => {
    setSelectedDrink([
      ...selectedDrink,
      {
        drinkId: newSelected.id,
        quantity: 1,
        price: newSelected.price,
        name: newSelected.name,
        imagePath: newSelected.image,
        note: "",
      },
    ]);
  };
  ...
  const deleteOutSelected = (deleteSelected) => {
    const updatedSelectedDrink = selectedDrink.filter(
      (item) => item.drinkId !== deleteSelected.id
    );
    setSelectedDrink(updatedSelectedDrink);
  };
  const deleteOutSelectedOnOrder = (deleteSelected) => {
    const updatedSelectedDrink = selectedDrink.filter(
      (item) => item.drinkId !== deleteSelected.drinkId
    );
    setSelectedDrink(updatedSelectedDrink);
  };
  const clearSelected = () => {
