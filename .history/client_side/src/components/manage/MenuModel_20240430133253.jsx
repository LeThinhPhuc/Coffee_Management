...
const MenuModel = (props) => {
    ...
    useEffect(() => {
        if (props.item && props.item.ingredients) {
            const initialIngredients = props.item.ingredients.map((ing) => {
                const foundIngredient = ingredients.find(
                    (ingredient) => ingredient.name === ing.ingredientName
                );
                return {
            ...
            setIngreArray(initialIngredients);
    ...
    const handleAddIngre = () => {
        const ingredient = ingredients.find((ing) => ing.id === ingreSelected);

        const existingIngredientIndex = ingreArray.findIndex(
            (ing) => ing.item.id === ingreSelected
        );

        if (existingIngredientIndex >= 0) {
            // Ingredient already exists, update the quantity
            const updatedIngreArray = [...ingreArray];
            updatedIngreArray[existingIngredientIndex] = {
                ...updatedIngreArray[existingIngredientIndex],
                quantity:
                    parseInt(
                        updatedIngreArray[existingIngredientIndex].quantity,
                        10
                    ) + parseInt(ingreAmount, 10),
            };
            setIngreArray([...updatedIngreArray]);
        } else {
            // Ingredient does not exist, add new
            const dataAdd = {
                item: ingredient,
                quantity: ingreAmount,
            };
            setIngreArray((ingreArray) => [...ingreArray, dataAdd]);
        }

        // setIngreAmount(1);
        // setIngreSelected(ingredients[0].id || "");
    };

    const handleDeleteIngre = (item) => {
        const updatedIngredients = ingreArray.filter(
            (ingredient) => ingredient.item.id !== item.id
        );
        setIngreArray(updatedIngredients);
    };
    ...
    const handleSubmit = () => {
        const validationErrors =
            name === "" || info === "" || image === null || /[^0-9]/.test(info);
        const ingredientCheck =
            props.item.type === "menu" && ingreArray.length === 0;

        if (validationErrors || ingredientCheck) {
        ...
        if (props.item.type === "menu") {
            ...
            const drinkData = {
                id: isAdd ? "string" : props.item.id, // Assuming 'string' is a placeholder
                name: name,
                price: parseFloat(info),
                imagePath: image,
                drinkTypeId: drinkTypeId,
                ingredients: ingredients,
            };

            console.log(isAdd ? "Adding drink" : "Updating drink", drinkData);
            const action = isAdd ? addDrink(drinkData) : updateDrink(drinkData);
            dispatch(action);
        } else {
            const entryDateObj = new Date(entryDate);
            const expiryDateObj = new Date(expiryDate);

            if (entryDateObj >= expiryDateObj) {
            ...
            const action = isAdd
                ...
                : updateIngredient(props.item.id, ingredientData);
            console.log(
                isAdd ? "Adding ingredient" : "Updating ingredient",
                ingredientData
            );
            dispatch(action);
        }
                        ...
                        <form className="relative p-6 flex flex-col gap-5">
                            ...
                            {props.type == "menu" && ( //! Nguyên liệu, [....]
                                <div className="relative">
                                    ...
                                    {/* //* Detail */}
                                    <div
                                        className="z-50 absolute w-[300px] h-[300px] bg-white -right-[330px] -bottom-[125px] border-0 rounded-lg shadow-lg
                                    p-5"
                                    >
